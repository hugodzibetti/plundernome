...
export class ScrollManager {
  ...
  loadMore(): void { this.renderedCount += CHUNK_SIZE }
  reset(): void { this.renderedCount = CHUNK_SIZE }
  getRange(): { start: number; end: number } { return { start: 0, end: this.renderedCount } }
}

export function buildFlowView(box: GtkFlowBox): { clamp: AdwClampScrollable } {
  const clamp = createScrollContent(box)
