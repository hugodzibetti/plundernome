...
export const CatalogView = GObject.registerClass(
  ...
  class CatalogView extends Adw.Bin {
    ...
    constructor() {
      ...
      searchRow.append(this.searchBar);
      searchRow.append(this.filterSpinner);
      ...
      const f = buildFilterToolbar((st) => {
        this.filterState = st;
        this.scheduleFilter();
      });
      this.filterState = f.state;
      this.filterEmit = f.emit;
      this.sourceBox = f.sourceBox;
      searchHeader.append(f.bar);
      outer.append(searchHeader);
      this.viewStack = new Gtk.Stack();
      this.viewStack.set_transition_type(Gtk.StackTransitionType.CROSSFADE);
      this.flowBox = new Gtk.FlowBox();
      this.flowBox.set_max_children_per_line(4);
      this.flowBox.set_min_children_per_line(2);
      this.flowBox.set_homogeneous(false);
      this.flowBox.set_selection_mode(Gtk.SelectionMode.NONE);
      const gv = buildFlowView(this.flowBox);
      this.gridClamp = gv.clamp;
      this.viewStack.add_named(gv.clamp, 'grid');
      this.listBox = new Gtk.ListBox({ css_classes: ['boxed-list', 'catalog-list'] });
      const lv = buildListView(this.listBox);
      this.listClamp = lv.clamp;
      this.viewStack.add_named(lv.clamp, 'list');
      this.scrollMgr.connectScroll(this.gridClamp, () => this.loadMore());
      this.scrollMgr.connectScroll(this.listClamp, () => this.loadMore());
    ...
    private filterAndRender(): void {
      ...
      this.renderChunk();
      this.updateEmptyState(this.allFiltered.length);
      this.updateResultCount(this.allFiltered.length);
    }
    ...
    private updateEmptyState(n: number): void {
      ...
      const q = this.searchEntry.get_text().toLowerCase();
      let child: unknown = this.emptyPage.get_first_child();
      let idx = 0;
      while (child) {
    ...
    setLoading(loading: boolean): void {
      if (loading) {
        ...
        this.mainStack.add_named(this.loadingOverlay, 'loading');
        this.mainStack.set_visible_child_name('loading');
      } else if (this.loadingOverlay) {
        this.mainStack.remove(this.loadingOverlay);
        this.loadingOverlay = null;
      }
    }
    showError(msg: string): void {
      if (!this.errorPage) {
        this.errorPage = buildErrorPage(msg);
        this.mainStack.add_named(this.errorPage, 'error');
      }
      this.errorPage.set_description(msg);
      this.mainStack.set_visible_child_name('error');
    }
    private showGameDetail(gameId: string): void {
      const game = this.games.find((g) => g.id === gameId);
      if (game) showDetailDialog({ game, parent: this, onDownload: () => this.dlHandler?.(game.id) });
    }
  },
);
