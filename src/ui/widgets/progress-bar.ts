...
function statusText(d: Download): string {
  ...
  return { queued: _t('downloads.status.queued'), verifying: _t('downloads.status.verifying'), paused: _t('downloads.status.paused'), resuming: _t('downloads.status.resuming'), completed: _t('downloads.status.completed') }[d.status] ?? d.status
}

export const ProgressBarWidget = GObject.registerClass({
  GTypeName: 'ProgressBarWidget',
}, class ProgressBarWidget extends imports.gi.Adw.Bin {
  ...
  constructor(download: Download) {
    ...
    const topRow = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL, spacing: 8 })
