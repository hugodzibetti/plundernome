...
export function createSpinRow(props: {
  ...
  const adj = new Gtk.Adjustment({ lower: props.min, upper: props.max, step_increment: props.step ?? 1, value: props.value })
...
export function createDetailDialog(props: {
  ...
  dialog.add_response('download', props.downloadLabel ?? 'Download')
...
export function createMenuPopover(items: Array<{
  label: string
  onClick: () => void
}>, parent: GtkWidget): GtkPopoverMenu {
  ...
  for (const item of items) {
    const btn = createButton({ label: item.label, cssClass: 'flat', onClick: () => { item.onClick(); popover.popdown() } })
    box.append(btn)
  }
