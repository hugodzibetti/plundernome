...
export const LibraryView = GObject.registerClass({
  ...
  constructor() {
    super(); this.add_css_class('library-view')
    this.listBox = new Gtk.ListBox({ css_classes: ['boxed-list'] })
    const listBoxWrap = new Gtk.Box({ orientation: Gtk.Orientation.VERTICAL, spacing: 8 })
    const headerBox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL, spacing: 8 })
    headerBox.append(createButton({ iconName: 'view-refresh-symbolic', tooltip: _t('library.refresh'), onClick: () => this.refreshHandler?.() }))
    this.sortDropdown = createSortDropdown((key) => {
      this.sortKey = key; this.applySort()
      this.sortChangedHandler?.(key)
    })
    headerBox.append(this.sortDropdown)
    listBoxWrap.append(headerBox)
    listBoxWrap.append(this.listBox)
    this.importBtn = createButton({ label: _t('library.steam-import'), tooltip: _t('library.steam-import.tooltip'), cssClass: 'suggested-action', onClick: () => this.steamImportHandler?.() })
    listBoxWrap.append(this.importBtn)
    this.libraryClamp = createScrollContent(listBoxWrap)
    this.stack = new Gtk.Stack(); this.set_child(this.stack)
    const emptyIcon = new Gtk.Label({ label: '📚' })
    emptyIcon.add_css_class('empty-icon')
    const emptyTitle = new Gtk.Label({ label: _t('library.empty.title'), xalign: 0 })
    emptyTitle.add_css_class('empty-title')
    const emptyDesc = new Gtk.Label({ label: _t('library.empty.description'), xalign: 0, wrap: true })
    emptyDesc.add_css_class('empty-desc')
    this.emptyPage = new Gtk.Box({ orientation: Gtk.Orientation.VERTICAL, spacing: 12, halign: Gtk.Align.CENTER, valign: Gtk.Align.CENTER })
    this.emptyPage.add_css_class('empty-state')
    this.emptyPage.append(emptyIcon)
    this.emptyPage.append(emptyTitle)
    this.emptyPage.append(emptyDesc)
    const catalogBtn = createButton({ label: _t('library.empty.action'), cssClass: 'suggested-action', onClick: () => this.openCatalogHandler?.() })
    this.emptyPage.append(catalogBtn)
    this.stack.add_child(this.emptyPage); this.stack.add_child(this.libraryClamp)
    this.stack.set_visible_child(this.emptyPage)
  }

  onImportFolder(cb: (path: string) => void): void { this.importHandler = cb }
  onSteamImport(cb: () => void): void { this.steamImportHandler = cb }
  onRefreshLibrary(cb: () => void): void { this.refreshHandler = cb }
  ...
  showImportDialog(): void {
    ...
    if (!win) return
    createFilePicker({
      action: 'select-folder', title: _t('library.select-folder'), parent: win,
  ...
  private renderGames(): void {
    ...
    while (row) { const next = row.get_next_sibling(); this.listBox.remove(row); row = next as GtkWidget | null }
    const sorted = sortEntries(this.cachedGames, this.sortKey)
    if (sorted.length === 0) { this.stack.set_visible_child(this.emptyPage); return }
    this.stack.set_visible_child(this.libraryClamp)
    for (const { game, profile, playtime, protonRating } of sorted) {
      this.listBox.append(buildGameRow(game, profile, playtime, this.launchOptionsHandler!, this.playHandler!, this.removeHandler!, protonRating, this.addToAppMenuHandler ?? undefined, this.removeFromAppMenuHandler ?? undefined))
    }
  }
  ...
  setSortKey(key: SortKey): void { this.sortKey = key; this.sortDropdown.set_selected(SORT_OPTS.findIndex(o => o.key === key)); this.applySort() }
