...
1. Agent reads: docs/AGENTS.md + FEATURE.md + types.ts + deps' types.ts
...
├── docs/         AI agent docs + project docs
├── scripts/      Build, tests, conventions check
├── config/       Build configs, docker, lint rules
├── server/       Docker-based HTTP server
├── flatpak/      Flatpak packaging manifests
└── data/         Desktop + schema files
```
