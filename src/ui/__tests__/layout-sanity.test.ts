...
function parseLayoutRules(): Map<string, { halign: string; valign: string }> {
  ...
  while ((m = regex.exec(md)) !== null) {
    if (m[1] && m[2] && m[3]) {
      const w = m[1].trim().toLowerCase().replace(/\s+.*$/, '')
...
describe('UI structural invariants', () => {
  ...
  describe.each(giFiles)('%s', (filepath) => {
    ...
    it('no Gtk.ScrolledWindow usage', () => {
      expect(content.includes('new Gtk.ScrolledWindow'),
        `File uses Gtk.ScrolledWindow — use createScrollContent from templates`).toBe(false)
    })

    it('no extends Gtk.Window', () => {
      expect(/extends\s+(?:imports\.gi\.)?Gtk\.Window\b/.test(content),
        `File extends Gtk.Window — should extend Adw.Bin or Adw.ApplicationWindow`).toBe(false)
    })

    it('no `as any` casts in view/widget files', () => {
      expect(/\bas\s+any\b/.test(content),
        `File uses 'as any' cast — narrow with unknown instead`).toBe(false)
    })

    it('no set_size_request with -1 for width', () => {
      expect(/set_size_request\(\s*-1\s*,/.test(content),
        `File uses set_size_request(-1, ...) — width must be explicit`).toBe(false)
    })

    it('every file has at least one add_css_class call', () => {
      expect(content.includes('add_css_class'),
        `File has no add_css_class call — every view/widget needs >=1 CSS class`).toBe(true)
    })

    it('no bare Gtk.Button constructor (use factory)', () => {
      expect(/new\s+(?:imports\.gi\.)?Gtk\.Button\s*\(/.test(content),
        `File uses 'new Gtk.Button()' — use createButton() from factory.ts`).toBe(false)
    })

    it('no .show() on dialogs (use .present())', () => {
      expect(/\.show\s*\(\s*\)/.test(content),
        `File uses .show() — use .present() instead`).toBe(false)
    })
    ...
    it('prefers template imports over direct widget construction', () => {
      ...
      const usesTemplate = content.includes('templates/') || content.includes('createScrollContent') ||
        content.includes('createGridContent') || content.includes('createListContent')
      ...
      if (!isCatalogView && !isCatalogRender && !isErrorLog && (usesFlowBox || usesListBox)) {
        expect(usesTemplate,
          `File uses direct Gtk.FlowBox/ListBox — use createGridContent/createListContent from templates`).toBe(true)
      }
    })
    ...
    it('halign/valign usage follows LAYOUT.md charter (warning only)', () => {
      ...
      const widgetType = fileBase.includes('card') ? 'button' :
        fileBase.includes('row') ? 'button' :
        fileBase.includes('badge') ? 'label' :
        'box'
...
describe('GObject.registerClass conventions', () => {
  ...
  it('every registerClass call has matching GTypeName and class name', () => {
    for (const file of allFiles) {
      ...
      const matches = content.matchAll(/GObject\.registerClass\(\{(?:[^}]*)\s*GTypeName:\s+'(\w+)'\s*[^}]*\},\s+class\s+(\w+)/g)
      for (const m of matches) {
        if (m[1] && m[2]) {
          expect(m[1],
            `${relative(UI_DIR, file)}: GTypeName '${m[1]}' must match class name '${m[2]}'`).toBe(m[2])
        }
      }
    }
  })
