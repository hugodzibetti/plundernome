...
declare class GtkWidget {
  ...
  get_style_context(): GtkStyleContext;
...
declare class GtkCssProvider {
  ...
  load_from_string(data: string): void;
}

declare class GtkStyleContext {
  add_provider(provider: GtkCssProvider, priority: number): void;
}
...
interface SoupServer {
  add_handler(
    path: string,
    callback: (server: SoupServer, msg: SoupMessage, path: string, query: Record<string, string> | null) => void,
  ): void;
  ...
  gi: {
    ...
    GLib: {
      ...
      io_add_watch(
        ...
        priority: number,
        condition: number,
        callback: (source: GioSocket, condition: number) => boolean,
      ): number;
