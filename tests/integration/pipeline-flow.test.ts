...
import { createGameID, validateGame, createInitialPipelineState, isValidPipelineTransition } from '../../src/domain/functions'
...
describe('reducePipeline full flow', () => {
  ...
  it('advances through all valid steps', () => {
    const steps: PipelineStep[] = ['verifying', 'extracting', 'detecting-deps', 'installing-deps', 'finding-exe', 'registering', 'completed']
...
describe('reducePipeline error handling', () => {
  ...
  it('resets failed pipeline and starts fresh', () => {
    let s: PipelineState = { ...createInitialPipelineState('retry-game'), status: 'failed', errorMessage: 'Network error' }
...
describe('getPipelineSteps', () => {
  it('returns all 8 steps in order', () => {
    ...
    expect(steps).toEqual([
      ...
      'detecting-deps', 'installing-deps',
      'finding-exe', 'registering', 'completed',
    ])
  })
})
...
describe('extractDownloadLinks', () => {
  ...
  it('extracts torrent file links', () => {
    const html = '<a href="https://example.com/game.torrent">torrent</a> <a href="https://example.com/patch.torrent">patch</a>'
  ...
  beforeAll(() => {
    ...
    for (const article of articles) {
      ...
      scraped.push({
        ...
        name, sourceId: 'fitgirl', sourceGameId, url,
        description: article.match(/<p[^>]*>([^<]*)<\/p>/i)?.[1]?.trim() ?? '',
        size: '', sizeBytes: 0,
        lastUpdated: new Date().toISOString(),
        downloadType: 'torrent', tags: [],
      })
    }
  })
