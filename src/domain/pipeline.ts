...
import { isValidPipelineTransition } from './functions'
...
export function serializePipelineState(state: PipelineState): { step: string; status: string; progress: number; errorMessage?: string } {
  return { step: state.step, status: state.status, progress: state.progress, errorMessage: state.errorMessage }
}

export function deserializePipelineState(gameId: GameID, data: { step: string; status: string; progress: number; errorMessage?: string }): PipelineState {
  return { gameId, step: data.step as PipelineStep, status: data.status as PipelineState['status'], progress: data.progress, errorMessage: data.errorMessage }
}
