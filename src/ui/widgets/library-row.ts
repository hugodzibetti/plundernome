...
export function buildGameRow(
  ...
  pic.add_css_class('library-cover');
  pic.add_css_class('sizing-row-cover');
  ...
  row.add_suffix(
    createButton({
      ...
      tooltip: _t('library.launch-options'),
      onClick: () => launchOptionsHandler(game.id),
    }),
  );
  row.add_suffix(
    createButton({
      iconName: 'media-playback-start-symbolic',
      tooltip: _t('library.play'),
      cssClass: 'suggested-action',
      onClick: () => playHandler(game.id),
    }),
  );
  const removeBtn = createButton({
    iconName: 'user-trash-symbolic',
    tooltip: _t('library.remove'),
    cssClass: 'destructive-action',
    onClick: () => {
      const win = row.get_native();
      if (!win) {
        removeHandler(game.id);
        return;
      }
      createAlertDialog({
        heading: _t('library.remove.confirm.title').replace('{0}', game.name),
        body: _t('library.remove.confirm.body'),
        confirmLabel: _t('library.remove'),
  ...
  if ((addToAppMenuHandler || removeFromAppMenuHandler) && game.installPath) {
    ...
    gesture.connect('pressed', () => {
      ...
      if (removeFromAppMenuHandler)
        items.push({ label: _t('library.remove-from-app-menu'), onClick: () => removeFromAppMenuHandler(game.id) });
