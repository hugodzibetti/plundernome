...
export function setPipelineSteps(
  ...
  const found = timelineMap.get(downloadId) as InstanceType<typeof PipelineTimelineWidget> | undefined;
  if (found) {
    found.setSteps(steps);
  }
}

export function removePipelineTimeline(id: string, timelineMap: Map<string, unknown>): void {
  const timeline = timelineMap.get(id);
  if (timeline) {
    const parent = (timeline as GtkWidget).get_parent();
    if (parent) (parent as GtkContainer).remove(timeline);
