...
import { createGameID } from '../../domain/functions'
import {
  extractElAmigosDownloadLinks, extractGloadDownloadLinks, extractOvaGamesDownloadLinks,
} from '../html-parser-sources-new'

const elamigosSource = {
  id: 'elamigos', name: 'ElAmigos Games',
  baseUrl: 'https://www.elamigosgamez.net', scrapeType: 'html',
  updateIntervalMinutes: 360, enabled: true,
} as const satisfies SourceDefinition

const gloadSource = {
  id: 'gload', name: 'GLOAD.to',
  baseUrl: 'https://gload.to', scrapeType: 'html',
  updateIntervalMinutes: 360, enabled: true,
} as const satisfies SourceDefinition

const ovaSource = {
  id: 'ovagames', name: 'Ova Games',
  baseUrl: 'https://www.ovagames.com', scrapeType: 'html',
  updateIntervalMinutes: 360, enabled: true,
} as const satisfies SourceDefinition
...
function parseElAmigosArticle(article: string, source: SourceDefinition): Game | null {
  ...
  const sizeBytes = sizeMatchNum ? Math.round(parseFloat(sizeMatchNum[1]!) * ({ 'GB': 1024**3, 'MB': 1024**2, 'KB': 1024 })[sizeMatchNum[2]!.toUpperCase()]!) : 0
  ...
  return {
    ...
    name, sourceId: source.id, sourceGameId, url,
    description: '', size: sizeStr, sizeBytes,
    lastUpdated: new Date().toISOString(),
    downloadType: 'torrent', imageUrl, tags: [],
  }
}
...
function parseGloadArticle(article: string, source: SourceDefinition): Game | null {
  ...
  return {
    ...
    name, sourceId: source.id, sourceGameId, url,
    description: '', size: sizeStr, sizeBytes,
    lastUpdated, downloadType: 'torrent', imageUrl, tags,
  }
}
...
function parseOvaGamesPost(post: string, source: SourceDefinition): Game | null {
  ...
  const imageUrl = imgMatch && !imgMatch[1].includes('readmore') && !imgMatch[1].includes('logo') ? imgMatch[1] : undefined
  const sourceGameId = url.match(/\/([^/]+)\/?$/)?.[1] ?? url
  return {
    id: createGameID(source.id, sourceGameId),
    name, sourceId: source.id, sourceGameId, url,
    description: '', size: '', sizeBytes: 0,
    lastUpdated: new Date().toISOString(),
    downloadType: 'torrent', imageUrl, tags: [],
  }
}
...
describe('parseGloadGames regex fallback', () => {
  const article1 = '<article id="post-83273" class="game-archive-card post-83273 category-pc genre-action">'
  ...
  const article2 = '<article id="post-83270" class="game-archive-card post-83270 category-pc genre-simulation genre-sport">'
  ...
  it('parses size from card-size div', () => {
    ...
    expect(games[1]?.sizeBytes).toBe(Math.round(35.20 * 1024 * 1024 * 1024))
  })
