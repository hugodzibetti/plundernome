...
export function showRetryWithMirrorDialog(
  ...
  const body = `Download failed for URL:\n${currentUrl}\n\n${remaining} mirror${remaining !== 1 ? 's' : ''} available. Retry with next mirror?`
  ...
  dialog.add_response('retry', 'Retry with next mirror')
...
export function showErrorDialog(opts: ErrorDialogOptions): void {
  ...
  if (stackTrace) {
    const expander = new Gtk.Expander({ label: 'Stack Trace', expanded: false })
