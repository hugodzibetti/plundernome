...
export const DownloadRowWidget = GObject.registerClass({
...
}, class DownloadRowWidget extends imports.gi.Adw.Bin {
  ...
  constructor(download: Download, handler: DownloadRowHandler | null = null) {
    ...
    this.pauseBtn = createButton({ iconName: 'media-playback-pause-symbolic', tooltip: _t('downloads.pause'), onClick: () => this.handler?.('pause', this.download.id) })
    this.resumeBtn = createButton({ iconName: 'media-playback-start-symbolic', tooltip: _t('downloads.resume'), onClick: () => this.handler?.('resume', this.download.id) })
    this.moveUpBtn = createButton({ iconName: 'go-up-symbolic', tooltip: _t('downloads.move-up'), onClick: () => this.handler?.('reorder', this.download.id, -1) })
    this.moveDownBtn = createButton({ iconName: 'go-down-symbolic', tooltip: _t('downloads.move-down'), onClick: () => this.handler?.('reorder', this.download.id, 1) })
